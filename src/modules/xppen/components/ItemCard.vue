...
<template>
    <div class="flex flex-col gap-2 shadow-xl border border-slate-300 rounded-lg overflow-hidden">
        ...
        <div class="p-2">
            <h2 class="text-xl font-bold my-2">{{ item.name }}</h2>
            ...
            <div class="flex justify-between items-center mt-5">
                <span class="text-amber-700 text-lg font-bold" v-if="item.price_usd == 0">{{item.price_mmk}} MMK</span>
                <span class="text-amber-700 text-lg font-bold flex items-center gap-2" v-else>{{item.price_usd}} <DollarSign class="text-green-700" :size="20" /></span>
